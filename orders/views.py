...
from .models import Category, Table, Order, OrderItem, MenuItem, KitchenStation
import json
...
def kitchen_dashboard_live_view(request, station_code):
    ...
    orderitems =  OrderItem.objects.filter(menu_item__station__code=station_code).exclude(
        Q(status=OrderItem.ITEM_STATUS.READY) | Q(status=OrderItem.ITEM_STATUS.SERVED )
    ).order_by("-priority")
